...
class BackupScreen extends ConsumerWidget{
      ...
      body: ListView(
        children:[
          ListTile(
            ...
            trailing: busy ? const CircularProgressIndicator()
            : null,
            enabled: !busy,
            onTap:()
            async{
              final tempFile =
              await ref.read(backupNotifierProvider.notifier)
              .export();
              ...
              if( savedPath == null)
              {
                // user cancelled — not an error, don't show a snackbar return;
              }
              try{
                if(
                  !(
              ...
              catch(e)
              {
                ...
                .showSnackBar(
                  const SnackBar(
                    content: Text('An error occured while saving the backup')
                  ),
                );
              }
            },
          ...
          ListTile(
            ...
            async{
              ...
              if( path == null) return;
              final ok = await ref.read(backupNotifierProvider.notifier)
              .import(path);
              if(
                !ok &&
                context.mounted
  ...
  {
    final message =
    ref.read(backupNotifierProvider)
